...
# # Load the JSON file
# def summarize():
#   with open('genai.json', 'r') as file:
#     data = json.load(file)
#   ARTICLE = data.get('Twitter', {}).get('terms_and_conditions', '')


# # Tokenize the article and split into chunks
...
#     summary_ids = model.generate(chunk.unsqueeze(0), max_length=500, min_length=30, do_sample=False)
...
from gemini import GenerativeModel,GenerationConfig
...
def summarize_with_gemini(text):
  ...
  model = GenerativeModel('gemini-1.5-flash',generation_config=GenerationConfig(temperature=0.5))
